...
const IntervieweeChat: React.FC = () => {
  ...
  const [finished, setFinished] = useState(interviewFinished);

  useEffect(() => {
    if (!interviewStarted || finished) return;
  ...
  }, [currentIndex, finished, interviewStarted]);
  ...
  if (!interviewStarted) return null; // do not render before starting
  if (finished)
    return (
      <Card title="Interview Finished">
  ...
  return (
    <Card title={`Question ${currentIndex + 1} of ${questions.length}`} style={{ marginBottom: "20px" }}>
      ...
      <div style={{ marginTop: "10px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Progress
          percent={Math.round((1 - timeLeft / questions[currentIndex].time) * 100)}
